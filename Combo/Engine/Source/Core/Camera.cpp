...
void Camera::Drone()
{
	...
	float moveX = sin(glm::radians(Camera::Orientation.y)) * 0.5f;
	float moveZ = cos(glm::radians(Camera::Orientation.y)) * 0.5f;
	float moveY = sin(glm::radians(Camera::Orientation.x)) * 0.5f;
