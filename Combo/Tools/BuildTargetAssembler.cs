...
namespace Combo.Tools {
    public class BuildTargetAssembler {
        ...
        public static Dictionary<string,Combo.BuildTools.BuildTarget> generateAssembliesFromTargets() {
            Dictionary<String,Combo.BuildTools.BuildTarget> assemblies = new Dictionary<String,Combo.BuildTools.BuildTarget>();
            foreach(string path in Directory.GetFiles("./", "*.cs", SearchOption.AllDirectories)) {
                if(path.EndsWith("Target.cs")) {
                    ...
                    foreach(Type clazz in assembly.GetTypes()) {
                        if(clazz.IsSubclassOf(typeof(Combo.BuildTools.BuildTarget)))
                        {
                            object instance = Activator.CreateInstance(clazz);
                            Combo.BuildTools.BuildTarget target = (Combo.BuildTools.BuildTarget)instance;
            ...
            for(int i = 0;i<assemblies.Count;i++) {
                ...
                Combo.BuildTools.BuildTarget entryValue = entry.Value;
                if(entry.Value.GetModules().Count != 0) {
                    foreach(string module in entry.Value.GetModules()) {
                        //Find the module by name
                        Combo.BuildTools.BuildTarget target = null;
